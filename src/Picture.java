...
public class Picture extends SimplePicture {
	...
	public void mirrorDiagonal(){
		...
		for (int row = 0; row < height / 2; row++) {
			for (int col = 0; col < width / 2; col++) {
				botLeftPixel = pixels[height - 1 - row][col];
				topRightPixel = pixels[row][width - 1 - col];
	...
	public void myCollage(){
		...
		Picture swanNoBlue = new Picture(wall);
		swanNoBlue.zeroBlue();
		this.copy(swanNoBlue, 300, 0);
	...
	public void edgeDetectionTopToBot(int edgeDist) {
		Pixel topPixel = null;
		Pixel botPixel = null;
		Pixel[][] pixels = this.getPixels2D();
		Color botColor = null;
	...
	public void edgeDetectionAlg(int edgeDist){
		...
		for (int row = 0; row < pixels.length ; row++) {
			for (int col = 0; col < (pixels[0].length - 1) ; col++) {
				...
				botRightPixel = pixels[row][col + 1];
				rightColor = botRightPixel.getColor();
				if (topLeftPixel.colorDistance(rightColor) > edgeDist)
